...
class App extends React.Component<unknown, AppState> {
        ...
        return (
            <div className="App">
                ...
                <section>
                    ...
                    <p>
                        ...
                        <button onClick={this.getPlaylists}>Get playlists</button>
                                { playlists && <select onChange={this.getPlaylist}>
                                    {playlists.map((playlist) => <option key={playlist.name} value={playlist.id}>{playlist.name}</option>)}
                                </select>}
                    </p></> :
                    <p>
                ...
                <section>
                    <h2>Support</h2>
                    { artists &&
                    artists.map((artist) => <button key={artist} title={artist} onClick={this.findArtist}>{artist}</button> )
                    }
                </section>
            </div>
        );
    ...
    private clearState = () => {
        this.setState({artists: undefined, playlists: undefined, selectedPlaylist: undefined});
    }
    ...
    private authenticate = () => {
        ...
        const scopes = ['playlist-read-private', 'playlist-read-collaborative', 'user-top-read', 'user-read-recently-played'];
        const state = `123`;
        window.location.href = `https://accounts.spotify.com/authorize?client_id=f3dfad56fac44b20ab4d43bf912c29ce&redirect_uri=${redirectUri}&scope=${scopes.join('%20')}&response_type=token&state=${state}`;
    }
    ...
    private getPlaylists = async() => {
        ...
        const playlists = response.items.map(itemResponse => ({name: itemResponse.name, id: itemResponse.id}));
